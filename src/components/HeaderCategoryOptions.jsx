    ...
    <Container className="mt-6">
      ...
      >
        {visitOptions.map((option) => (
          <Link key={option.title} href={`/category/${encodeURIComponent(option.title)}`} passHref>
            <div
              ...
              ${option.title === activePage ? "text-darkColor font-bold" : "text-black"} 
            ...
            >
              ...
              <Label className="mt-2 text-xs sm:text-sm font-semibold">{option.title}</Label>
            </div>
          </Link>
        ))}
      </div>
